// SegmentedControl 分段控制器 — 带滑动指示器动画
import React, { useEffect, useRef } from 'react';
import {
  View, Text, TouchableOpacity, Animated,
  StyleSheet, ViewStyle, LayoutChangeEvent,
} from 'react-native';
...
export const SegmentedControl: React.FC<SegmentedControlProps> = ({
  ...
  const paddingHoriz = spacing.xs;

  useEffect(() => {
    const idx = Math.max(0, activeIndex);
    if (segmentWidths.current.length > 0 && containerWidth.current > 0) {
      // 计算指示器位置
      const totalSpacing = paddingHoriz * 2;
      const availableWidth = containerWidth.current - totalSpacing;
      const segW = availableWidth / segments.length;

      // 每个 segment 中心位置
      const center = paddingHoriz + segW * idx + segW / 2;
      const indicatorWidth = segW * 0.85;

      Animated.spring(indicatorX, {
        toValue: center - indicatorWidth / 2,
        tension: 80,
        friction: 10,
...
const styles = StyleSheet.create({
  ...
  segment: {
    ...
    paddingVertical: spacing.sm - 1, zIndex: 1,
  },
  label: {
    fontSize: fontSize.sm, fontWeight: fontWeight.medium as any,
    color: colors.textSecondary,
  },
  labelActive: {
    color: colors.textInverse, fontWeight: fontWeight.semibold as any,
  },
